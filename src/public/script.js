...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const res = await fetch('/api/v1/auth/verify-pass-reset-token', {
    method: 'POST',
    headers: {
...
const handleSubmit = async (e) => {
  ...
  if (password.value.trim() !== confirmPassword.value) {
    return displayError('Passwords do not matched!');
  }
  ...
  const res = await fetch('/api/v1/auth/update-password', {
    ...
    body: JSON.stringify({ password: password.value, userId, token }),
  });
